...
def list_datasources(db: Session = Depends(get_db)):
    items = db.query(models.DataSource).order_by(models.DataSource.id).all()
    return [_ds_to_dict(d) for d in items]
...
def get_datasource(ds_id: int, db: Session = Depends(get_db)):
    ...
    return {
        **_ds_to_dict(ds),
        "table_meta": [_tm_to_dict(t) for t in tms]
    }
...
def list_datasources(
    ...
    return {
        "items": [_ds_to_dict(d) for d in items],
        "total": total,
        "skip": skip,
        "limit": limit
    }
...
def list_tablemeta(
    ...
    return {
        "items": [_tm_to_dict(t) for t in q],
        "total": total,
        "skip": skip,
        "limit": limit
    }
...
def get_tablemeta(tm_id: int, db: Session = Depends(get_db)):
    ...
    return {"items": [_ds_to_dict(d) for d in items], "total": total, "skip": skip, "limit": limit}tm_to_dict(tm)

@router.delete("/tablemeta/{tm_id}")
def delete_tablemeta(tm_id: int, db: Session = Depends(get_db)):
    tm = db.query(models.TableMeta).filter(models.TableMeta.id == tm_id).first()
    if not tm:
        raise HTTPException(status_code=404, detail="TableMeta not found")
    db.delete(tm)
...
def update_tablemeta(tm_id: int, payload: dict = Body(...), db: Session = Depends(get_db)):
    tm = db.query(models.TableMeta).filter(models.TableMeta.id == tm_id).first()
    if not tm:
        raise HTTPException(status_code=404, detail="TableMeta not found")
